> CORRECTION 2026-09-08 (batch 15, src/icytower/replay.c + src/icytower/game_data.c, verified offline AND against real files): S4's two open literals are closed, both read out of the image with pefile. The `.itr` magic at **VA 0x4d7dd0** is **`"ITR140"`** -- six bytes, NO terminator; the file-format version is 1.4.0 even in a 1.5.1 binary. The 7-byte tag at **VA 0x4d7a7e** that `create_replay` copies into `name` is **`"Harold"`** -- the game's default character, not a format marker. With those plus batch 14's write order, ALL THIRTEEN real `.itr` files in assets/profiles/MissingNO/replays parse with zero trailing bytes and `calc_replay_checksum` reproduces every one of their stored checksums, so the format is now confirmed against files the real game wrote, not only against the emulated original. Three further corrections from `load_replay` (0x41cde8) and `create_replay` (0x41cce8): the file is opened TWICE (pass 1 reads magic + count into stack locals and closes, because Allegro's PACKFILE has no seek); a VERIFIED replay is returned with its `checksum` field LEFT AT 0 (zeroed for the recomputation and never restored on the match path); and `create_replay` clears `name` twice and never clears `date`, so `date[8..31]` -- which the checksum hashes -- is uninitialised malloc memory. Full detail: PROMOTIONS.md batch 15.

> CORRECTION 2026-09-08 (batch 14, src/icytower/replay.c, verified offline): S4's INFERRED "a near-verbatim `Treplay` serialization" is wrong in three places. `save_replay` (0x41dd78) was fully traced this pass: `checksum` (offset 0x4c) is written near the END, after `comment` (0xa8), not in struct position; the five 100-float `tc_*` columns are written INTERLEAVED BY INDEX (c q t s f, c q t s f, ...) as 500 separate 4-byte writes, not as five arrays; and each `Trecord` is written REVERSED and SHORT -- `cycle_count` (4 bytes) then `key_flags` (1 byte), five bytes on disk against the struct's eight. `ccc[5]`/`jc[5]` are likewise ten separate 4-byte writes. Also: the 31-byte literal `"              ICYTOWERISGREAT "` copied into `date` at 0x41ddfe is NOT dead -- the sprintf that follows writes only 11 chars + NUL, so bytes 12..30 keep the tag, and `calc_replay_checksum` hashes all 32 bytes of `date`. And `calc_replay_checksum` (0x41bac4), left untraced by S4, hashes only `tc_c_data`/`tc_q_data`/`tc_t_data` -- `tc_s_data` and `tc_f_data` are written to disk and never checksummed. Full detail: PROMOTIONS.md batch 14.
